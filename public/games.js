...
function make_graph(game_id){
    $.get( "/game/"+game_id, function( data ) {
    buildGraph(prepareData(10800, 0,
        data.map(function(d){return d.points})), 
        "#graph-points", "points");
    buildGraph(prepareData(50800, 0,
        data.map(function(d){return d.time})), 
        "#graph-time", "time[s]");
    });     
}

function prepareData(max, min, data){
